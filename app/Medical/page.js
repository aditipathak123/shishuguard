...
export default function VaccineTracker({ babyId }) {
  ...
  useEffect(() => {
    document.title = "Medical |  ShishuGuard";
    if (isAuth) {
      fetchVaccines();
      // fetchBabyBirthDate()
    }
  ...
  return (
    <div className="p-6 md:p-6 md:mx-20 lg:mx-0 w-full space-y-6 mx-auto">
      ...
      <div className="text-center mt-10 text-lg font-semibold animate-pulse bg-gradient-to-r from-cyan-400 via-pink-400 to-purple-400 bg-clip-text text-transparent">
        For more information regarding this section, visit{" "}
        <a href="/Resources" className="font-bold text-cyan-400 hover:text-pink-400 transition-all duration-300 hover:underline">
          Resources
        </a>{" "}
        or{" "}
        <a href="/Faqs" className="font-bold text-purple-400 hover:text-cyan-400 transition-all duration-300 hover:underline">
          FAQs
        </a>
        .
      </div>
