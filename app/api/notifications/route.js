...
export async function POST(req) {
  try {
    ...
    if (!user?.user?.id) {
      return Response.json(
        {
          error: "Please login again to create notifications",
        },
        { status: 401 }
      );
    }
...
export async function GET(req) {
  try {
    ...
    if (!user?.user?.id) {
      return Response.json(
        {
          error: "Please login again to view notifications",
        },
        { status: 401 }
      );
    }
...
export async function PATCH(req) {
  try {
    ...
    if (!user?.user?.id) {
      return Response.json(
        {
          error: "Please login again to update notifications",
        },
        { status: 401 }
      );
    }
...
export async function DELETE(req) {
  try {
    ...
    if (!user?.user?.id) {
      return Response.json(
        {
          error: "Please login again to delete notifications",
        },
        { status: 401 }
      );
    }
